...
def yaml_docs(text: str) -> list[dict[str, Any]]:
    ...
    for match in SETUP_RE.finditer(text or ""):
        try: value = yaml.safe_load(match.group(1))
...
def setup_from_comments(comments: list[dict[str, Any]]) -> dict[str, Any] | None:
    for comment in reversed(comments):
        for doc in yaml_docs(comment.get("body") or ""):
            if isinstance(doc.get("examination_setup"), dict): return doc["examination_setup"]
    return None
...
def evidence_requirement(requirement_id: str, proposition: str, evidence_class: str, producer: str,
    ...
    return {"id": requirement_id, "proposition": proposition, "evidence_class": evidence_class,
            "producer": producer, "context_boundary": context_boundary, "required_fields": required_fields,
            "privacy_safe_capture_rule": privacy_safe_capture_rule, "acceptance_criterion": acceptance_criterion,
            "rerun_target": rerun_target, "priority": priority, "routing_target": routing_target}
...
def remediation_plan(issue_number: int, record: dict[str, Any]) -> dict[str, Any]:
    ...
    for iid in interactions:
        if any(f"no executable_evaluation rules for {iid}" in item for item in missing):
            requirements.append(evidence_requirement(
                f"DPIP-{iid}-EXECUTABLE-RULES", f"DPIP can mechanically evaluate the scoped {iid} privacy proposition against repository observation evidence.",
                "executable-fixture-rule", "DPIP", "repository-local fixture; does not assert upstream runtime behaviour",
                ...
                f"{iid} has executable evaluation rules and the repository-native evaluator can execute its canonical observation fixture deterministically.",
                f"DPIP examination #{issue_number} / interaction {iid}", "blocks one scoped interaction conclusion", "dpip-local"))
        if any(f"no unique repository observation fixture for {iid}" in item for item in missing):
            requirements.append(evidence_requirement(
                f"DPIP-{iid}-OBSERVATION-FIXTURE", f"DPIP has one unambiguous repository observation fixture capable of exercising the scoped {iid} failure mode.",
                "executable-observation-fixture", "DPIP", "repository-local fixture; A/B contexts where the interaction tests cross-context correlation",
    ...
    runtime_specs = [
        ("RELATIONSHIP-BINDER", "Relationship DID / edge identifiers are not stably reused or derivable across unrelated contexts.", "implementation-trace", "DTG relationship/credential implementation", ["normalized relationship/edge handle", "context label", "derivation/reuse indicator", "observer vantage"]),
        ("STATUS-DISCOVERY", "Status and policy-discovery activity does not expose a stable cross-context join handle.", "status-request-response-trace", "status/policy discovery implementation or operator", ["normalized request handle", "normalized response handle", "cache/log key", "currentness decision", "observer vantage"]),
        ("TRUST-TASK-RETENTION", "Retained Trust Task / relationship evidence does not create an unintended durable cross-context correlator.", "operator-retention-trace", "Trust Tasks implementation/operator", ["retained field names", "normalized retained handles", "retention horizon", "observer/access class", "context label"]),
        ("VERIFIER-CONTEXT", "The deliberate-correlation mechanism is bounded to the intended verifier/context and does not become a wider correlator.", "protocol-transcript", "verifier / ZKP presentation implementation", ["normalized context commitment", "verifier context", "transcript binding fields", "reuse/derivation indicator"]),
    ]
    for suffix, proposition, evidence_class, producer, fields in runtime_specs:
        requirements.append(evidence_requirement(
            f"RUNTIME-{suffix}", proposition, evidence_class, producer, "two unrelated execution contexts A and B", fields,
            ...
            f"DPIP examination #{issue_number} pinned to the original scope/source revisions", "blocks runtime privacy conclusion", "upstream-runtime"))

    plan = {"status": "required", "source_issue": issue_number, "setup_digest": record.get("setup_digest"),
            "execution_digest": record.get("execution_digest"), "requirements": requirements,
...
def build_execution(issue_number: int, setup: dict[str, Any]) -> dict[str, Any]:
    catalog = profile_catalog(); checks = []; missing = []; mechanical_results = []
    interactions = [str(i) for i in setup.get("candidate_interactions", []) or []]
    for iid in interactions:
        profile_path = catalog.get(iid)
        if not profile_path:
            missing.append(f"missing canonical interaction fixture: {iid}"); continue
        evidence_path = observation_for(iid)
        if evidence_path is None:
            missing.append(f"no unique repository observation fixture for {iid}")
            checks.append({"interaction": iid, "profile": str(profile_path), "status": "NOT_RUN", "reason": "observation fixture unavailable"}); continue
        ...
        if not has_executable_evaluation(profile):
            ...
            checks.append({"interaction": iid, "profile": str(profile_path), "evidence": str(evidence_path), "status": "NOT_RUN", "reason": "executable rules unavailable"}); continue
        try: outputs = evaluate(profile, evidence)
        except Exception as exc:
            missing.append(f"execution failed for {iid}: {exc}")
            checks.append({"interaction": iid, "profile": str(profile_path), "evidence": str(evidence_path), "status": "INDETERMINATE", "reason": str(exc)}); continue
    ...
    runtime_scoped = set(interactions) & {"C3", "C5"}
    if runtime_scoped:
        missing.append("upstream runtime A/B evidence not supplied for " + ", ".join(sorted(runtime_scoped)))

    state = "evidence-ready" if checks and not missing else "evidence-incomplete"
    execution = {"execution_evidence": {"status": state, "source_issue": issue_number, "setup_digest": setup.get("setup_digest"),
        "candidate_interactions": setup.get("candidate_interactions", []), "candidate_profiles": setup.get("candidate_profiles", []),
        "checks": checks, "mechanical_results": mechanical_results, "missing_evidence": missing,
        "source_pins": setup.get("source_pins", []), "privacy_judgment": "not-made", "human_acceptance_required": True}}
...
def conclusion_from_execution(execution: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if record.get("status") != "evidence-incomplete": return None
    ...
    return {"dpip_examination": {"applicability": "applicable", "conclusion": "INDETERMINATE",
        "affected_interactions": record.get("candidate_interactions", []), "affected_claims": [], "affected_invariants": [],
        "evidence_summary": summary, "residual_correlation": "Cross-context correlation cannot be ruled in or out from source-backed evidence alone.",
        "action": action, "evidence_remediation_plan": plan,
        "human_summary": {"outcome": "We do not have enough evidence to decide yet", "explanation": summary, "action": action},
        "source_pins": record.get("source_pins", []), "execution_digest": record.get("execution_digest"), "human_acceptance_required": True}}
...
def publish(repo: str, issue: dict[str, Any], token: str) -> None:
    number = int(issue["number"]); existing = comments(repo, number, token); setup = setup_from_comments(existing)
    if not setup: print(f"SKIP #{number}: no examination_setup comment"); return
    evidence = build_execution(number, setup); digest = evidence["execution_evidence"]["execution_digest"]
    ...
    if conclusion is not None:
        ...
        try: api("DELETE", repo, f"issues/{number}/labels/run%3Ain-progress", token)
...
def self_test() -> int:
    ...
    assert incomplete["status"] == "evidence-incomplete", incomplete
    assert all(check["status"] == "EXECUTED" for check in incomplete["checks"]), incomplete
    assert not any("no executable_evaluation rules for C3" in item for item in incomplete["missing_evidence"])
    assert not any("no unique repository observation fixture for C5" in item for item in incomplete["missing_evidence"])
    assert any("upstream runtime A/B evidence" in item for item in incomplete["missing_evidence"])
    ...
    assert not any(r["routing_target"] == "dpip-local" for r in plan["requirements"]), plan
...
def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__); parser.add_argument("--self-test", action="store_true"); parser.add_argument("--issue-number", type=int); args = parser.parse_args()
